...
from reportlab.lib.utils import asBytes, asNative, int2Byte, rawBytes
...
class StandardEncryption:
    ...
    def __init__(
        self, userPassword, ownerPassword=None, canPrint=1, canModify=1, canCopy=1, canAnnotate=1, strength=None
    ):
        """
    ...
    def info(self):
        ...
        return StandardEncryptionDictionary(
            O=self.O, OE=self.OE, U=self.U, UE=self.UE, P=self.P, Perms=self.Perms, revision=self.revision
        )
...
def encryptionkey(password, OwnerKey, Permissions, FileId1, revision=None):
    ...
    if DEBUG >= 10:
        print(
            ...
            % tuple([hexText(str(x)) for x in (password, OwnerKey, Permissions, FileId1, revision, key)])
        )
    return key
...
def computeU(encryptionkey, encodestring=PadString, revision=None, documentId=None):
    ...
    if DEBUG >= 10:
        print(
            "computeU(%s,%s,%s,%s)==>%s"
            % tuple([hexText(str(x)) for x in (encryptionkey, encodestring, revision, documentId, result)])
        )
    return result
...
def encodePDF(key, objectNumber, generationNumber, string, revision=None):
    ...
    if DEBUG >= 10:
        print(
            ...
            % tuple([hexText(str(x)) for x in (key, objectNumber, generationNumber, string, revision, encrypted)])
        )
    return encrypted
...
def encryptCanvas(
    canvas, userPassword, ownerPassword=None, canPrint=1, canModify=1, canCopy=1, canAnnotate=1, strength=40
):
...
def encryptDocTemplate(
    dt, userPassword, ownerPassword=None, canPrint=1, canModify=1, canCopy=1, canAnnotate=1, strength=40
):
...
def encryptPdfInMemory(
    inputPDF, userPassword, ownerPassword=None, canPrint=1, canModify=1, canCopy=1, canAnnotate=1, strength=40
):
    ...
    try:
        from rlextra.pageCatcher.pageCatcher import restoreFormsInMemory, storeFormsInMemory
    except ImportError:
        raise ImportError(
...
def encryptPdfOnDisk(
    ...
    outputPDF = encryptPdfInMemory(
        inputPDF, userPassword, ownerPassword, canPrint, canModify, canCopy, canAnnotate, strength=strength
    )
...
"""
    ...
    if len(argv) > 0:
        ...
        filesize = encryptPdfOnDisk(
            infile, SAVEFILE, USER, OWNER, PRINTABLE, MODIFIABLE, COPYPASTABLE, ANNOTATABLE, strength=STRENGTH
        )
