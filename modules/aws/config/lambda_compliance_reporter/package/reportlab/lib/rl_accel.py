...
if _py_funcs:
    ...
    from reportlab.lib.utils import asBytes, asNative, isSeq, isUnicode, rawBytes
...
if "instanceStringWidthT1" in _py_funcs:
    ...
    def _py_instanceStringWidthT1(self, text, size, encoding="utf8"):
        ...
        return (
            sum((sum(map(f.widths.__getitem__, t)) for f, t in _py_unicode2T1(text, [self] + self.substitutionFonts)))
            * 0.001
            * size
        )
...
if "asciiBase85Decode" in _py_funcs:
    ...
    def _py_asciiBase85Decode(input):
        ...
        if remainder_size > 0:
            ...
            c4 = ord(lastbit[3]) - 33
            c5 = ord(lastbit[4]) - 33
            num = (((85 * c1 + c2) * 85 + c3) * 85 + c4) * 85 + (c5 + (0, 0, 0xFFFFFF, 0xFFFF, 0xFF)[remainder_size])
            temp, b4 = divmod(num, 256)
            temp, b3 = divmod(temp, 256)
            b1, b2 = divmod(temp, 256)
            assert num == 16777216 * b1 + 65536 * b2 + 256 * b3 + b4, "dodgy code!"
