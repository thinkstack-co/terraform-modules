...
class UntrustedAstTransformer(ast.NodeTransformer):
    ...
    def gen_attr_check(self, node, attr_name):
        ...
        call_getattr = ast.Call(
            func=ast.Name("__rl_getattr__", ast.Load()), args=[node, ast.Constant(attr_name)], keywords=[]
        )
    ...
    def visit_Attribute(self, node):
        ...
        if isinstance(node.ctx, ast.Load):
            node = self.visit_children(node)
            new_node = ast.Call(
                func=ast.Name("__rl_getattr__", ast.Load()), args=[node.value, ast.Constant(node.attr)], keywords=[]
            )
        ...
        if isinstance(node.target, ast.Attribute):
            ...
            new_node = ast.Assign(
                ...
                value=ast.Call(
                    ...
                    args=[ast.Constant(augOps[type(node.op)]), ast.Name(node.target.id, ast.Load()), node.value],
                    keywords=[],
                ),
            )
    ...
    def visit_BinOp(self, node):
        ...
        if isinstance(op, (ast.Mult, ast.Add, ast.Pow)):
            opf = (
                "__rl_mult__" if isinstance(op, ast.Mult) else "__rl_add__" if isinstance(op, ast.Add) else "__rl_pow__"
            )
...
class __RL_SAFE_ENV__:
    ...
    def __rl_is_allowed_name__(self, name, crash=True):
        ...
        if isinstance(name, strTypes):
            if name not in self.allowed_magic_names and (
                ...
                or (name.startswith("__") and name != "__" and name not in self.allowed_magic_methods)
            ):
                if not crash:
                    return False
    ...
    def __rl_safe_eval__(
        self, expr, g, l, mode, timeout=None, allowed_magic_methods=None, __frame_depth__=3, allowed_magic_names=None
    ):
        ...
        if allowed_magic_methods is not None:
            self.allowed_magic_methods = (
                (__allowed_magic_methods__ if allowed_magic_methods == True else allowed_magic_methods)
