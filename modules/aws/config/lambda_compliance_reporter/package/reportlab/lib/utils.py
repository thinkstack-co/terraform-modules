...
from reportlab.lib.rltempfile import get_rl_tempdir, get_rl_tempfile

from .rl_safe_eval import rl_extended_literal_eval, rl_safe_eval, rl_safe_exec, safer_globals
        ...
        try:
            ...
            try:
                if trustedHosts is not None:
                    purl = urlparse(name)
                    if purl[0] and not (
                        (purl[0] in ("data", "file") or trustedHosts.match(purl[1])) and (purl[0] in trustedSchemes)
                    ):
