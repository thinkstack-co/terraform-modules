...
class paragraphEngine:
    ...
    def format(self, maxwidth, maxheight, program, leading=0):
        ...
        while remainder and room:  # heightremaining>=self.leading and remainder:
            ...
            try:
                (lineIsFull, line, cursor, currentLength, usedIndent, maxLength, justStrings) = self.fitLine(
                    remainder, maxwidth
                )
            except:
...
def readLength(text):
    ...
    try:
        ...
        try:
            multiplier = {"in": 72, "cm": 28.3464566929, "mm": 2.83464566929, "pt": 1}[  # 72/2.54; is this accurate?
                units
            ]
        except KeyError:
            raise ValueError("invalid length attribute '%s'" % text)
...
def EmbedInRml2pdf():
    ...
    from rlextra.rml2pdf.rml2pdf import Controller, MapNode  # may not need to use superclass?
