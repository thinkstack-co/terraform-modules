...
from reportlab.lib.textsplit import ALL_CANNOT_START, wordSplit
...
from reportlab.platypus.paraparser import _PCT, ParaParser
from reportlab.platypus.paraparser import _num as _parser_num
from reportlab.platypus.paraparser import _re_us_value
from reportlab.rl_config import _FUZZ, decimalSymbol, hyphenationMinWordLength, paraFontSizeHeightOffset
...
def _hyphenateFragWord(
    hyphenator, FW, newWidth, maxWidth, uriWasteReduce, embeddedHyphenation, hymwl=hyphenationMinWordLength
):
    ww = FW[0]
    if ww == 0:
        return []
    if len(FW) == 2:
        ...
        if isinstance(FW, _SplitFragLL):
            s = _SplitWordLL(s)
        R = _hyGenPair(
            hyphenator, s, ww, newWidth, maxWidth, f.fontName, f.fontSize, uriWasteReduce, embeddedHyphenation, hymwl
        )
        if R:
...
def _hyphenateWord(
    ...
    if ww == 0:
        return []
    R = _hyGenPair(
        hyphenator, w, ww, newWidth, maxWidth, fontName, fontSize, uriWasteReduce, embeddedHyphenation, hymwl
    )
    if R:
        hy, hylen, hw, tw, h, t = R
        if isBidiStr(w):
            return [bidiStrWrap((_SplitWordHY if hy else _SplitWordH)(h + hy), w), bidiStrWrap(_SplitWordEnd(t), w)]
        else:
...
class Paragraph(Flowable):
    ...
    def _get_split_blParaFunc(self):
        return (
            ...
            else (_split_blParaHard if not _processed_frags(self.frags) else self._split_blParaProcessed)
        )
                ...
                if not text:
                    ...
                    if doBidi:
                        ...
                        if shaping:
                            ...
                            words = [
                                innerBidiStrWrap(shapeStr(_, fontName=f.fontName, fontSize=v.fontSize), VX[i], i)
                    ...
                    if splitLongWords and not (isinstance(word, _SplitWord) or forcedSplit):
                        ...
                        if wordWidth > max(maxWidths[nmw : nmw + 1]):
                            ...
                            words[0:0] = _splitWord(
                                word, currentWidth + spaceWidth, maxWidths, lineno, fontName, fontSize, self.encoding
                            )
