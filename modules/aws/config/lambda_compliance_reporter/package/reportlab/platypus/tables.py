...
from reportlab.lib.styles import ParagraphStyle, PropertySet, _baseFontName
from reportlab.lib.utils import __UNSET__, IdentStr, annotateException, asNative, flatten, isStr, strTypes
...
from reportlab.platypus.flowables import Flowable, LIIndenter, Preformatted
...
only rows may be strings with values in {_SPECIALROWS!r}"""
    ...
    def _drawLines(self):
        ...
        if rrd:  # we are collection some lines
            ...
            def rcCanvLine(xs, ys, xe, ye):
                if (xs == xe and (xs >= rrd.x1 or xs <= rrd.x0)) or (  # vertical line that needs to be saved
                ...
                if (sr < n and er >= n) or (doInRowSplit and sr == n):
                    ...
                    if op == "GRID":
                        if doInRowSplit:
                            ...
                            A.append(
                                ("LINEBELOW", (sc, n - 1), (ec, n - 1), weight, color, cap, dash, join, count, space)
                            )
                ...
                for linecmd in self._linecmds:
                    ...
                    if (
                        ...
                        if op in ("GRID", "INNERGRID"):
                            newlinecmds.append(
                                ("INNERGRID", (sc, sr), (ec, n - 1), weight, color, cap, dash, join, count, space)
                            )
                            newlinecmds.append(
                                ("INNERGRID", (sc, n), (ec, er), weight, color, cap, dash, join, count, space)
                            )
                            newlinecmds.append(
                                ("LINEBELOW", (sc, n - 1), (ec, n - 1), weight, color, cap, dash, join, count, space)
                            )
    ...
    def _restoreRoundingObscuredLines(self):
        ...
        for xs, ys, xe, ye, weight, color, cap, dash, join in SL:
            ...
            if ys == ye:
                ...
                # we have some rounding so we must use a path
                c0 = (
                    _quadrantDef(xpos, "bottom", (xs, ys), r0, kind=2, direction="bottom-top") if ys < y0 + r0 else None
                )
        ...
        if not self._spanCmds:
            ...
            if renderCB:
                for rowNo, (row, rowstyle, rowpos, rowheight) in enumerate(
                    zip(self._cellvalues, self._cellStyles, self._rowpositions[1:], self._rowHeights)
                ):
                    ...
                    for colNo, (cellval, cellstyle, colpos, colwidth) in enumerate(
                        ...
                        renderCB(
                            self, "startCell", rowNo, colNo, cellval, cellstyle, (colpos, rowpos), (colwidth, rowheight)
                        )
        ...
        for cmd, (sc, sr), (ec, er), arg in self._bkgrndcmds:
            ...
            if hasattr(arg, "__call__"):
                ...
                if (
                    ...
                    and arg[0]
                    in ("VERTICAL", "HORIZONTAL", "VERTICAL2", "HORIZONTAL2", "LINEARGRADIENT", "RADIALGRADIENT")
                ):
    ...
    def _drawCell(self, cellval, cellstyle, pos, size):
        ...
        if cellstyle.destination:
            # external hyperlink
            self.canv.linkRect(
                "", cellstyle.destination, Rect=(colpos, rowpos, colpos + colwidth, rowpos + rowheight), relative=1
            )
...
BOX_STYLE = TableStyle([("BOX", (0, 0), (-1, -1), 0.50, colors.black), ("ALIGN", (1, 1), (-1, -1), "RIGHT")])
