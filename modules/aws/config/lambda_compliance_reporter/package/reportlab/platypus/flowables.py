...
from reportlab.lib.utils import annotateException, rl_safe_exec, strTypes
from reportlab.pdfbase import pdfutils
from reportlab.pdfbase.pdfmetrics import stringWidth
from reportlab.rl_config import _FUZZ, ignoreContainerActions, listWrapOnFakeWidth, overlapAttachedSpace
...
class Image(Flowable):
    ...
    def __init__(
        self, filename, width=None, height=None, kind="direct", mask="auto", lazy=1, hAlign="CENTER", useDPI=False
    ):
...
def _listWrapOn(F, availWidth, canv, mergeSpace=1, obj=None, dims=None, fakeWidth=None):
    ...
    if cframe:
        from reportlab.platypus.doctemplate import Indenter, _addGeneratedContent
    ...
    def drawOn(self, canv, x, y, _sW=0):
        if self._width > _FUZZ and self._height > _FUZZ:
            ...
            if st or self._fillColor:
                ...
                canv.rect(
                    self._x, self._y, self._width, self._height, stroke=1 if st else 0, fill=1 if self._fillColor else 0
                )
                canv.restoreState()
...
class BalancedColumns(_FindSplitterMixin, NullDraw):
    ...
    def _generated_content(self, aW, aH):
        G = []
        frame = self._frame
        from reportlab.platypus.doctemplate import ActionFlowable, FrameBreak, Indenter, LayoutError
    ...
    def wrap(self, aW, aH):
        ...
        if aH < self.spaceBefore + self._needed - _FUZZ:
            ...
            if not self_frame:
                ...
                self._frame = Frame(0, 0, aW, 0x7FFFFFFF, leftPadding=0, rightPadding=0, topPadding=0, bottomPadding=0)
...
class FrameBG(AnchorFlowable):
    ...
    def __init__(
        self, color=None, left=0, right=0, start=True, strokeWidth=None, strokeColor=None, strokeDashArray=None
    ):
        Spacer.__init__(self, 0, 0)
        self.start = start
        if start:
...
class FrameSplitter(NullDraw):
    ...
    def wrap(self, aW, aH):
        frame = self._frame
        from reportlab.platypus.doctemplate import CurrentFrameFlowable, LayoutError, NextPageTemplate
...
class LIIndenter(DDIndenter):
    ...
    def split(self, aW, aH):
        ...
        if len(S) > 1 and hasattr(f, "blPara"):
            ...
            if (getattr(getattr(f, "style", None), "alignment", None) == 4 and nnl >= (cnl - 2)) or nnl == (cnl - 1):
                S = []
        return (
            [
                LIIndenter(
