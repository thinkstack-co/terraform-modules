...
from reportlab.pdfbase.pdfmetrics import getFont as pdfmetrics_getFont
from reportlab.pdfbase.pdfmetrics import stringWidth as pdfmetrics_stringWidth
from reportlab.pdfbase.pdfmetrics import unicode2T1 as pdfmetrics_unicode2T1
from reportlab.pdfbase.ttfonts import ShapeData, ShapedStr, _sdGuardL, shapeStr
    ...
    def _formatText(self, text):
        ...
        try:
            ...
            if font._dynamicFont:
                            ...
                            if cluster is None or sd.cluster < 0 or r != self._rise:
                                if cluster is not None:
                                    ...
                                    A = [
                                        v
                                        for v in (
                                            (("(%s)" % canv_escape(b"".join(g))) if k else fp_str(sum(g)))
                                            for k, g in groupby(
                                                filter(None, A.__self__), lambda x: isinstance(x, bytes)
                                            )
                                        )
                                        if v != "0"
                                    ]
                ...
                R(
                    "%s %s Tf %s TL"
                    % (canv._doc.getInternalFontName(font.fontName), fp_str(self._fontsize), fp_str(self._leading))
                )
                    ...
                    try:
                        ...
                        raise UnicodeDecodeError(
                            *(
                                ...
                                + (
                                    "%s\n%s-->%s<--%s"
                                    % (e.args[4], text[max(i - 10, 0) : i], text[i:j], text[j : j + 10]),
                                )
                            )
                        )
