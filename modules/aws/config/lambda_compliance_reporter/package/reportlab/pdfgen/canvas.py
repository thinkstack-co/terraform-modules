...
from reportlab.lib.colors import CMYKColor, Color, _chooseEnforceColorSpace, black, toColor
from reportlab.lib.rl_accel import escapePDF, fp_str
from reportlab.lib.utils import ImageReader, _digester, asUnicode, isSeq, isStr, isUnicode
...
class Canvas(_PDFColorSetter):
    ...
    def drawImage(
        ...
        x, y, width, height, scaled = aspectRatioFix(
            preserveAspectRatio, anchor, x, y, width, height, imgObj.width, imgObj.height, anchorAtXY
        )
    ...
    def highlightAnnotation(
        self, contents, Rect, QuadPoints=None, Color=[0.83, 0.89, 0.95], addtopage=1, name=None, relative=0, **kw
    ):
        """
    ...
    def radialGradient(self, x, y, radius, colors, positions=None, extend=True):
        ...
        shading = PDFRadialShading(
            x, y, 0.0, x, y, radius, Function=fcn, ColorSpace=colorSpace, Extend=_gradientExtendStr(extend)
        )
        self.shade(shading)
