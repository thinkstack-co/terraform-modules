...
from reportlab.pdfbase.pdfdoc import PDFDictionary, PDFName, PDFObject, PDFStream, PDFString
...
class AcroForm(PDFObject):
    ...
    def selectField(self, canvas, title, value, options, xmin, ymin, xmax, ymax):
        ...
        fontsize = canvas._fontsize
        field = SelectField(
            title, value, options, xmin, ymin, xmax, ymax, page, font=font, fontsize=fontsize, R=R, G=G, B=B
        )
        self.fields.append(field)
        canvas._addAnnotation(field)
...
FormResourcesDictionaryPattern = ["<<", " /Encoding ", ["Encoding"], "\r\n", " /Font ", ["Font"], "\r\n", ">>"]
...
def FormFont(BaseFont, Name):
    ...
    return PDFPattern(
        FormFontPattern, BaseFont=PDFName(BaseFont), Name=PDFName(Name), Encoding=PDFPattern(PDFDocEncodingPattern)
    )
...
def SelectField(
    title, value, options, xmin, ymin, xmax, ymax, page, font="Helvetica-Bold", fontsize=9, R=0, G=0, B=0.627
):
    # print "ARGS", (title, value, options, xmin, ymin, xmax, ymax, page, font, fontsize, R, G, B)
    from reportlab.pdfbase.pdfdoc import PDFArray, PDFName, PDFString
