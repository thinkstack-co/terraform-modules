...
from reportlab.graphics.shapes import STATE_DEFAULTS, Drawing, Group, Line, Polygon, PolyLine, Rect
from reportlab.graphics.widgetbase import PropHolder, TypedPropertyCollection
from reportlab.graphics.widgets.markers import isSymbol, makeMarker, uSymbol2Symbol
...
class StrandProperty(PropHolder):
    _attrMap = AttrMap(
        ...
        symbol=AttrMapValue(
            EitherOr((isStringOrNone, isSymbol)), desc="Widget placed at data points.", advancedUsage=1
        ),
...
class SpiderChart(PlotArea):
    ...
    def makeSwatchSample(self, rowNo, x, y, width, height):
        ...
        if fillColor is None and strokeColor is not None and strokeWidth > 0:
            bg = Line(
                x, ym, x + width, ym, strokeWidth=strokeWidth, strokeColor=strokeColor, strokeDashArray=strokeDashArray
            )
        ...
        for row in data:
            ...
            for i in range(n):
                ...
                L = self._innerDrawLabel(
                    strandLabels[(rowIdx, i)], r, cx, cy, angle, car, sar, labelClass=self.labelClass("strand")
                )
                if L:
                    labs.append(L)
