...
from reportlab.graphics.shapes import STATE_DEFAULTS, Drawing, Group, Line, Rect, String, _baseGFontName
from reportlab.graphics.widgetbase import PropHolder, TypedPropertyCollection, Widget
from reportlab.graphics.widgets.markers import isSymbol, uSymbol2Symbol
from reportlab.lib import colors
from reportlab.lib.attrmap import *
from reportlab.lib.utils import asNative, find_locals, isSeq, isStr
...
class Legend(Widget):
    ...
    _attrMap = AttrMap(
        ...
        swatchMarker=AttrMapValue(
            NoneOr(AutoOr(isSymbol)), desc="None, Auto() or makeMarker('Diamond') ...", advancedUsage=1
        ),
        ...
        dividerOffsX=AttrMapValue(
            SequenceOf(isNumber, emptyOK=0, lo=2, hi=2), desc="divider lines X offsets", advancedUsage=1
        ),
        ...
        for i in range(n):
            if autoCP:
                ...
                if isAuto(swatchMarker):
                    ...
                    col.index = i
                if isAuto(name):
                    name = getattr(swatchMarker, "chart", getattr(swatchMarker, "obj", None)).getSeriesName(
                        i, "series %d" % i
                    )
            T = _getLines(name)
            S = []
            ...
            for k, lines in enumerate(T):
                ...
                for t in lines:
                    aS(
                        String(
                            xoffs + scdx, y + scdy + vAdy, t, fontName=fN, fontSize=fS, fillColor=fC, textAnchor=anchor
                        )
                    )
                    y -= fL
                yd = min(yd, y)
            ...
            if isAuto(col):
                ...
                if isSymbol(swatchMarker):
                    ...
                    c = self._defaultSwatch(
                        swatchX, swatchY, dx, dy, fillColor=col, strokeWidth=strokeWidth, strokeColor=strokeColor
                    )
...
class TotalAnnotator(LegendColEndCallout):
    ...
    def __call__(self, legend, g, x, xt, y, width, lWidth):
        ...
        if self.lText and fillColor:
            g.add(
                String(
                    xt, yt, self.lText, fontName=fontName, fontSize=fontSize, fillColor=fillColor, textAnchor="start"
                )
            )
        if self.rText:
            g.add(
