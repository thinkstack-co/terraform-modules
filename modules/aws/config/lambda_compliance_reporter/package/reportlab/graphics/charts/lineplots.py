...
from reportlab.graphics.charts.axes import AdjYValueAxis, NormalDateXValueAxis, XValueAxis, YValueAxis
from reportlab.graphics.charts.textlabels import Label
from reportlab.graphics.charts.utils import *
from reportlab.graphics.shapes import Drawing, Group, Polygon, PolyLine, Rect, _SetKeyWordArgs
from reportlab.graphics.widgetbase import PropHolder, TypedPropertyCollection, tpcGetItem
from reportlab.graphics.widgets.grids import DoubleGrid, Grid, ShadedPolygon
...
class LinePlotProperties(PropHolder):
    _attrMap = AttrMap(
        ...
        lineStyle=AttrMapValue(
            NoneOr(OneOf("line", "joinedLine", "bar")), desc="What kind of plot this line is", advancedUsage=1
        ),
...
class LinePlot(AbstractLineChart):
    ...
    def makeLines(self):
        ...
        if (
            ...
            or [rowNo for rowNo in range(len(P)) if getattr(lines[rowNo % styleCount], "inFill", False)]
        ):
            ...
            if lineStyle == "bar":
                ...
                for x, y in row:
                    ...
                    if x < xmin:
                        ...
                        w -= xmax - x
                    g.add(
                        Rect(x, _y0, w, y - _y0, strokeWidth=strokeWidth, strokeColor=strokeColor, fillColor=fillColor)
                    )
...
class ScatterPlot(LinePlot):
    ...
    def __init__(self):
        ...
        self.x = self.x + _maxWidth(
            str(self.yValueAxis.valueMax), self.yValueAxis.labels.fontName, self.yValueAxis.labels.fontSize
        )
    ...
    def _getDrawingDimensions(self):
        ...
        tx = tx + (
            5
            * _maxWidth(str(self.yValueAxis.valueMax), self.yValueAxis.labels.fontName, self.yValueAxis.labels.fontSize)
        )
