...
class WedgeProperties(PropHolder):
    ...
    _attrMap = AttrMap(
        ...
        label_angle=AttrMapValue(
            isNumber, desc="Angle of the label, default (0) is horizontal, 90 is vertical, 180 is upside down"
        ),
...
class Pie(AbstractPieChart):
    _attrMap = AttrMap(
        ...
        sideLabelsOffset=AttrMapValue(
            isNumber, desc="The fraction of the pie width that the labels are situated at from the edges of the pie"
        ),
...
from reportlab.graphics.charts.utils3d import _2rad, _180_pi, _360, _getShaded
    ...
    def _fillSide(self, L, i, angle, strokeColor, strokeWidth, fillColor):
        ...
        if rd < self.rad_dist(self._sl3d[i].mid):
            ...
            L.append(
                (
                    rd,
                    Polygon(p, strokeColor=strokeColor, fillColor=fillColor, strokeWidth=strokeWidth, strokeLineJoin=1),
                )
            )
