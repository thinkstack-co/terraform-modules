...
from reportlab.graphics.charts.textlabels import DirectDrawFlowable, Label, PMVLabel, XLabel
...
from reportlab.graphics.widgetbase import TypedPropertyCollection, Widget
...
class CategoryAxis(_AxisG):
    ...
    _attrMap = AttrMap(
        ...
        labelAxisMode=AttrMapValue(
            OneOf("high", "low", "axis", "axispmv"), desc="Like joinAxisMode, but for the axis labels"
        ),
        ...
        if getattr(self, "_bubblePlot", None):
            ...
            if oMin is None:
                valueMin = self._cValueMin = _findMin(
                    dataSeries, self._dataIndex, 0, special=special, extraMinMaxValues=self.extraMinMaxValues
                )
            if oMax is None:
...
class AdjYValueAxis(YValueAxis):
    ...
    _attrMap = AttrMap(
        ...
        leftAxisSkipLL0=AttrMapValue(
            EitherOr((isBoolean, isListOfNumbers)), desc="Skip/Keep lowest tick label when true/false.\nOr skiplist"
        ),
    ...
    def _rangeAdjust(self):
        ...
        if abs(self._valueMin - 100) < 1e-6:
            ...
            m = max(
                self.leftAxisOrigShiftIPC * self._valueStep, (vMax - vMin) * self.leftAxisOrigShiftMin / self._length
            )
