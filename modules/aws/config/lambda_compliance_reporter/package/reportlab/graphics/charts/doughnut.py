...
from reportlab.graphics.shapes import Drawing, Group, Wedge
...
class Doughnut(AbstractPieChart):
    _attrMap = AttrMap(
        ...
        checkLabelOverlap=AttrMapValue(
            isBoolean, desc="If true check and attempt to fix\n standard label overlaps(default off)", advancedUsage=1
        ),
        sideLabels=AttrMapValue(
            isBoolean, desc="If true attempt to make chart with labels along side and pointers", advancedUsage=1
        ),
                        ...
                        if nshades > 1:
                            ...
                            for ish in range(nshades):
                                ...
                                if len(series) > 1:
                                    ...
                                    shSector = Wedge(
                                        cx, cy, xr, sha1, sha2, yradius=yr, radius1=xr1, yradius1=yr1, annular=True
                                    )
                ...
                if shader:
                    ...
                    if nshades > 1:
                        ...
                        for ish in range(nshades):
                            ...
                            if n > 1:
                                shSector = Wedge(
                                    cx, cy, xradius, sha1, sha2, yradius=yradius, radius1=xir, yradius1=yir
                                )
                            elif n == 1:
                                shSector = Wedge(
                                    cx,
