...
from reportlab.lib.utils import asBytes, isSeq
from reportlab.lib.validators import *
from reportlab.platypus import Flowable
from reportlab.rl_config import _unset_, decimalSymbol
from reportlab.rl_config import defaultGraphicsFontName as _baseGFontName
...
class Drawing(Group, Flowable):
    ...
    _xtraAttrMap = AttrMap(
        ...
        hAlign=AttrMapValue(
            OneOf("LEFT", "RIGHT", "CENTER", "CENTRE"), desc="Horizontal alignment within parent document"
        ),
        vAlign=AttrMapValue(
            OneOf("TOP", "BOTTOM", "CENTER", "CENTRE"), desc="Vertical alignment within parent document"
        ),
    ...
    _attrMap = AttrMap(
        BASE=Group,
        formats=AttrMapValue(SequenceOf(OneOf(*_saveModes), lo=1, emptyOK=0), desc="One or more plot modes"),
    )
...
class SolidShape(LineShape):
    ...
    _attrMap = AttrMap(
        ...
        fillOpacity=AttrMapValue(
            isOpacity, desc="the level of transparency of the color, any real number between 0 and 1"
        ),
...
def getArcPoints(
    centerx, centery, radius, startangledegrees, endangledegrees, yradius=None, degreedelta=None, reverse=None
):
    if yradius is None:
        yradius = radius
    points = []
...
class Hatching(Path):
    ...
    _attrMap = AttrMap(
        ...
        spacings=AttrMapValue(
            EitherOr((isNumber, isListOfNumbers, "spacings(s)")), desc="orthogonal distance(s) between hatching lines"
        ),
    )
...
class String(Shape):
    ...
    _attrMap = AttrMap(
        ...
        fontName=AttrMapValue(
            None, desc="font name of the text - font is either acrobat standard or registered when using external font."
        ),
        ...
        textAnchor=AttrMapValue(
            OneOf("start", "middle", "end", "numeric"), desc="treat (x,y) as one of the options below."
        ),
