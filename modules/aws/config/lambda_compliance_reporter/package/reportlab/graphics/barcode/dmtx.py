...
from reportlab.lib.validators import isBoxAnchor, isColor, isColorOrNone, isNumber, isString
    ...
    def recalc(self):
        ...
        matrix = list(
            filter(
                ...
                (
                    "".join(
                        (("x" if p[j : j + bpp] != b"\xff\xff\xff" else " ") for j in range(i, i + rowLen, cellLen))
                    ).strip()
