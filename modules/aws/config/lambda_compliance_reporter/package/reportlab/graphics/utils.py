...
from reportlab.lib.utils import isBytes, open_and_read, rl_exec
from reportlab.pdfbase.pdfmetrics import getFont, stringWidth, unicode2T1
from reportlab.pdfbase.ttfonts import ShapedStr

from .shapes import _PATH_OP_ARG_COUNT, _PATH_OP_NAMES, _baseGFontName, definePath
...
def __makeTextPathsCode__(tp=None, _TP=("freetype", "_renderPM")):
    from reportlab.rl_config import renderPMBackend, textPaths
...
def text2Path(
    text, x=0, y=0, fontName=_baseGFontName, fontSize=1000, anchor="start", truncate=1, pathReverse=0, gs=None, **kwds
):
