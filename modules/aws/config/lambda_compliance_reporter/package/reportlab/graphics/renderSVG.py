...
from reportlab.lib.utils import RLString, asNative, isBytes, isUnicode
...
class SVGCanvas:
    def __init__(self, size=(300, 300), encoding="utf-8", verbose=0, bom=False, **kwds):
        ...
        self._strokeColor = (
            self._fillColor
        ) = self._lineWidth = self._font = self._fontSize = self._lineCap = self._lineJoin = None
        ...
        self.groupTree = transformNode(
            self.doc, "g", id="group", transform="scale(1,-1) translate(0,-%d)" % self.height, **gtkw
        )
    ...
    def drawString(self, s, x, y, angle=0, link_info=None, text_anchor="left", textRenderMode=0, **_svgAttrs):
        ...
        if (self._fillColor != None and needFill) or (self._strokeColor != None and needStroke):
            ...
            text = transformNode(
                self.doc, "text", x=x, y=y, style=st, transform="translate(0,%d) scale(1,-1)" % (2 * y), **_svgAttrs
            )
    ...
    def translate(self, x, y):
        if (x, y) != (0, 0):
            self.currGroup.setAttribute(
                "transform", "%s %s" % (self.currGroup.getAttribute("transform"), "translate(%s)" % self.cfp_str(x, y))
            )

    def scale(self, sx, sy):
        if (sx, sy) != (1, 1):
            self.currGroup.setAttribute(
                "transform", "%s %s" % (self.groups[-1].getAttribute("transform"), "scale(%s)" % self.cfp_str(sx, sy))
            )
...
class _SVGRenderer(Renderer):
    ...
    def drawRect(self, rect):
        ...
        if rect.rx == rect.ry == 0:
            # plain old rectangle
            self._canvas.rect(
                rect.x, rect.y, rect.x + rect.width, rect.y + rect.height, link_info=link_info, **svgAttrs
            )
        else:
