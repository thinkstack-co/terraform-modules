...
from reportlab.graphics.shapes import Drawing, EmptyClipPath, Group, Line, LineShape, Rect, definePath
...
class Grid(Widget):
    ...
    _attrMap = AttrMap(
        ...
        orientation=AttrMapValue(
            OneOf(("vertical", "horizontal")), desc="Determines if stripes are vertical or horizontal."
        ),
        useLines=AttrMapValue(OneOf((0, 1)), desc="Determines if stripes are drawn with lines."),
        useRects=AttrMapValue(OneOf((0, 1)), desc="Determines if stripes are drawn with solid rectangles."),
        ...
        stripeColors=AttrMapValue(isListOfColors, desc="Colors applied cyclically in the right or upper direction."),
