from reportlab.graphics.shapes import Drawing, Group, Polygon, _DrawingEditorMixin
...
class AdjustableArrow(Widget):
    ...
    _attrMap = AttrMap(
        ...
        right=AttrMapValue(
            isBoolean, desc="If True (default) the arrow is horizontal pointing right\nFalse means it points up"
        ),
        angle=AttrMapValue(
            isNumber, desc="angle of arrow default (0), right True 0 is horizontal to right else vertical up"
        ),
    )
