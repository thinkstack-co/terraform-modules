...
from reportlab.lib.validators import OneOf, Validator, isColorOrNone, isNumber
...
class Marker(Widget):
    ...
    def _Circle(self):
        x, y = self.x + self.dx, self.y + self.dy
        s = Circle(
            x, y, self.size / 2.0, fillColor=self.fillColor, strokeColor=self.strokeColor, strokeWidth=self.strokeWidth
        )
        return s
