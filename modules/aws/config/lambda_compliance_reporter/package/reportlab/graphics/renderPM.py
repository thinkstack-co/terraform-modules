...
from reportlab.lib.utils import asUnicode, isUnicode
...
from .utils import RenderPMError
from .utils import setFont as _setFont
...
def Color2Hex(c):
    ...
    if c:
        return ((0xFF & int(255 * c.red)) << 16) | ((0xFF & int(255 * c.green)) << 8) | (0xFF & int(255 * c.blue))
    return c
...
class PMCanvas:
    ...
    def stringWidth(self, text, fontName=None, fontSize=None):
        return stringWidth(
            text, fontName or self._gs.fontName, (fontSize if fontSize is not None else self._gs.fontSize)
        )
