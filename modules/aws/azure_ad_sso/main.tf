...
data "aws_iam_policy_document" "thinkstack_azure_ad_assume_policy" {
  statement {
    ...
    principals {
      type        = "Federated"
      identifiers = ["${aws_iam_saml_provider.thinkstack_azure_ad_assume_policy.arn}"]
    }
...
resource "aws_iam_role" "role_admins" {
  assume_role_policy    = "${data.aws_iam_policy_document.thinkstack_azure_ad_assume_policy.arn}"
