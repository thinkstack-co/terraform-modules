...
from diagrams.aws.network import ELB, ALB, NLB, WAF, CloudFront
from diagrams.aws.storage import S3
from diagrams.aws.network import VPC as VPCIcon
...
class PublicSubnetNode(Node):
    ...
    def __init__(self, label):
        ...
        self.graph_attr = {"style": "filled", "fillcolor": "#e8f5e9", "color": "#4caf50"}
...
class PrivateSubnetNode(Node):
    ...
    def __init__(self, label):
        ...
        self.graph_attr = {"style": "filled", "fillcolor": "#e3f2fd", "color": "#2196f3"}
...
class VPCNode(Node):
    ...
    def __init__(self, label):
        ...
        self.graph_attr = {"style": "filled", "fillcolor": "#f3e5f5", "color": "#9c27b0"}
...
class EC2Node(Node):
    ...
    def __init__(self, label):
        ...
        self.graph_attr = {"style": "filled", "fillcolor": "#fff3e0", "color": "#ff9800"}
...
class ELBNode(Node):
    ...
    def __init__(self, label):
        ...
        self.graph_attr = {"style": "filled", "fillcolor": "#ffebee", "color": "#f44336"}
...
class WAFNode(Node):
    ...
    def __init__(self, label):
        ...
        self.graph_attr = {"style": "filled", "fillcolor": "#fce4ec", "color": "#e91e63"}
...
def get_target_health(elb, target_group_arn):
    try:
        return elb.describe_target_health(TargetGroupArn=target_group_arn)["TargetHealthDescriptions"]
    except Exception as e:
...
def lambda_handler(event, context):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with Diagram("AWS Network Architecture", filename=diagram_path, show=False, 
            ...
            waf_nodes = {}
            if web_acls:
                with Cluster("Web Application Firewalls", graph_attr={"bgcolor": "#fce4ec"}):
                    for acl in web_acls:
                        waf_nodes[acl["ARN"]] = WAF(acl["Name"])
            
            # Create S3 bucket nodes
            s3_nodes = {}
            regional_buckets = [b for b in s3_buckets if get_bucket_region(s3, b["Name"]) == region]
            if regional_buckets:
                with Cluster(f"S3 Buckets ({region})", graph_attr={"bgcolor": "#e8f5e9"}):
                    for bucket in regional_buckets[:10]:  # Limit to 10 buckets for readability
            ...
            for vpc in vpcs:
                ...
                with Cluster(f"VPC: {vpc_name}\n{vpc_cidr}", graph_attr={"bgcolor": "#f3e5f5"}):
                    ...
                    vpc_azs = sorted(set([s["AvailabilityZone"] for s in subnets if s["VpcId"] == vpc_id]))
                    
                    # Create load balancers for this VPC
                    vpc_lbs = [lb for lb in elbs if lb["VpcId"] == vpc_id]
                    if vpc_lbs:
                        with Cluster("Load Balancers", graph_attr={"bgcolor": "#ffebee"}):
                            for lb in vpc_lbs:
                    ...
                    # Process each AZ
                    for az in vpc_azs:
                        with Cluster(f"Availability Zone: {az}", graph_attr={"bgcolor": "#e3f2fd", "style": "dashed"}):
                            ...
                            if public_subnets:
                                with Cluster("Public Subnets", graph_attr={"bgcolor": "#e8f5e9"}):
                                    for subnet in public_subnets:
                                        ...
                                        with Cluster(f"{subnet_name}\n{subnet_cidr}"):
                                            subnet_nodes[subnet_id] = PublicSubnet(subnet_id)
                                            
                                            # EC2 instances in this subnet
                                            for reservation in instances:
                                                for instance in reservation["Instances"]:
                                                    if instance.get("SubnetId") == subnet_id and instance.get("State", {}).get("Name") == "running":
                                                        instance_name = get_instance_name(instance)
                                                        instance_type = instance.get("InstanceType", "")
                                                        instance_nodes[instance["InstanceId"]] = EC2(f"{instance_name}\n{instance_type}")
                            ...
                            if private_subnets:
                                with Cluster("Private Subnets", graph_attr={"bgcolor": "#e3f2fd"}):
                                    for subnet in private_subnets:
                                        ...
                                        with Cluster(f"{subnet_name}\n{subnet_cidr}"):
                                            subnet_nodes[subnet_id] = PrivateSubnet(subnet_id)
                    ...
                    if vpc_tgs:
                        with Cluster("Target Groups", graph_attr={"bgcolor": "#ffebee", "style": "dashed"}):
                            for tg in vpc_tgs:
                                tg_name = tg["TargetGroupName"]
                                tg_nodes[tg["TargetGroupArn"]] = TargetGroupNode(tg_name)
                ...
                if waf_arn in waf_nodes:
                    for lb_arn in lb_arns:
                        if lb_arn in lb_nodes:
                            waf_nodes[waf_arn] >> Edge(label="protects", style="dashed", color="red") >> lb_nodes[lb_arn]
                ...
                if lb_arn in lb_nodes:
                    for tg_arn in tg_arns:
                        if tg_arn in tg_nodes:
                            lb_nodes[lb_arn] >> Edge(label="forwards to", style="solid", color="blue") >> tg_nodes[tg_arn]
                ...
                if tg_arn in tg_nodes:
                    ...
                    for target in targets:
                        ...
                        if instance_id in instance_nodes:
                            ...
                            tg_nodes[tg_arn] >> Edge(label=health, style="dotted", color=color) >> instance_nodes[instance_id]
