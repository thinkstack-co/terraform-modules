...
| <a name="input_lifecycle_rules"></a> [lifecycle\_rules](#input\_lifecycle\_rules) | (Optional) Configuration of object lifecycle management (LCM). Can have several rules. | <pre>list(object({<br>    id     = string<br>    status = string<br>    rule   = object<br>  }))</pre> | `null` | no |
