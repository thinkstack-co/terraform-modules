      ...
      customize = [
        ...
        # Step 2: Install M365 Apps via Office Deployment Tool
        {
          ...
          inline = [
            ...
            "$configXml  = '<Configuration>' + [char]13 + [char]10",
            "$configXml += '  <Add OfficeClientEdition=\"64\" Channel=\"Current\">' + [char]13 + [char]10",
            "$configXml += '    <Product ID=\"O365ProPlusRetail\">' + [char]13 + [char]10",
            "$configXml += '      <Language ID=\"en-us\" />' + [char]13 + [char]10",
            "$configXml += '      <ExcludeApp ID=\"Teams\" />' + [char]13 + [char]10",
            "$configXml += '      <ExcludeApp ID=\"Groove\" />' + [char]13 + [char]10",
            "$configXml += '      <ExcludeApp ID=\"Lync\" />' + [char]13 + [char]10",
            "$configXml += '    </Product>' + [char]13 + [char]10",
            "$configXml += '  </Add>' + [char]13 + [char]10",
            "$configXml += '  <Property Name=\"SharedComputerLicensing\" Value=\"1\" />' + [char]13 + [char]10",
            "$configXml += '  <Property Name=\"PinIconsToTaskbar\" Value=\"FALSE\" />' + [char]13 + [char]10",
            "$configXml += '  <Display Level=\"None\" AcceptEULA=\"TRUE\" />' + [char]13 + [char]10",
            "$configXml += '  <Updates Enabled=\"TRUE\" />' + [char]13 + [char]10",
            "$configXml += '</Configuration>'",
            "$configXml | Set-Content -Path (Join-Path $odtPath 'config.xml') -Encoding UTF8",
