...
|------|-------------|------|---------|:--------:|
| <a name="input_backup_jobs"></a> [backup\_jobs](#input\_backup\_jobs) | List of backup jobs with associated vault details. | <pre>list(object({<br>    vault_name        = string # Name of the backup vault<br>    vault_kms_key_arn = string # ARN of the KMS key associated with the backup vault<br>    vault_tags        = string<br>    rule_name         = string<br>    schedule          = string<br>    retention_days    = number<br>  }))</pre> | `[]` | no |
