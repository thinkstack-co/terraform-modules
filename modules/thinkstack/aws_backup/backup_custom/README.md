...
|------|-------------|------|---------|:--------:|
| <a name="input_backup_jobs"></a> [backup\_jobs](#input\_backup\_jobs) | List of backup jobs with associated vault details. | <pre>list(object({<br>    vault_name     = string<br>    vault_tags     = map(string)<br>    rule_name      = string<br>    schedule       = string<br>    dr_region      = bool<br>    retention_days = number<br>    selection_tag  = string<br>  }))</pre> | `[]` | no |
