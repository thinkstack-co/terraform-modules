...
# Vault sort order
VAULT_SORT_ORDER = os.environ.get("VAULT_SORT_ORDER", "hourly,daily,weekly,monthly,yearly").split(",")
...
def get_resource_details(resource_arn):
    ...
    if len(parts) >= 6:
        ...
        if service in resource_type_map:
            if isinstance(resource_type_map[service], dict):
                friendly_type = resource_type_map[service].get(resource_type, f"{service.upper()} {resource_type}")
            else:
                friendly_type = resource_type_map[service]
        else:
...
def fetch_backup_jobs(vault_name, start_time, end_time):
...
def determine_backup_type(vault_name):
    """Determine backup type from vault name"""
    vault_lower = vault_name.lower()
    if "hourly" in vault_lower:
        return "Hourly"
    elif "daily" in vault_lower:
        return "Daily"
    elif "weekly" in vault_lower:
        return "Weekly"
    elif "monthly" in vault_lower:
        return "Monthly"
    elif "yearly" in vault_lower:
        return "Yearly"
    else:
        return "Custom"


def generate_pdf(backup_data, start_time, end_time, outfile):
    ...
    pdf.cell(0, 8, f"AWS Backup Status Report - Last {REPORT_DAYS} Day{'s' if REPORT_DAYS > 1 else ''}", ln=1, align="C")
    pdf.set_font("Arial", size=10, style="I")
    pdf.cell(0, 6, f"Period: {start_time.strftime('%Y-%m-%d %H:%M')} to {end_time.strftime('%Y-%m-%d %H:%M')} UTC", ln=1, align="C")
    pdf.ln(6)
    
    # Summary statistics
    total_jobs = sum(len(jobs) for jobs in backup_data.values())
    successful_jobs = sum(len([j for j in jobs if j["State"] == "COMPLETED"]) for jobs in backup_data.values())
    failed_jobs = sum(len([j for j in jobs if j["State"] == "FAILED"]) for jobs in backup_data.values())
    running_jobs = sum(len([j for j in jobs if j["State"] in ["RUNNING", "PENDING", "CREATED"]]) for jobs in backup_data.values())
    
    pdf.set_font("Arial", "B", 12)
    pdf.cell(0, 7, f"Total Backup Jobs: {total_jobs} | Successful: {successful_jobs} | Failed: {failed_jobs} | Running/Pending: {running_jobs}", ln=1, align="C")
    pdf.ln(4)
    
    # Table header
    col_widths = [35, 25, 30, 25, 30, 35, 20]  # Resource Name, Resource ID, Resource Type, Backup Type, Creation Time, Vault, Status
    headers = ["Resource Name", "Resource ID", "Resource Type", "Backup Type", "Creation Time", "Vault", "Status"]
    h = 7
    ...
    def get_vault_priority(vault_item):
    ...
    for vault_name, jobs in sorted(backup_data.items(), key=get_vault_priority):
        ...
        sorted_jobs = sorted(jobs, key=lambda x: x.get("CreationDate", datetime.datetime.now()), reverse=True)
        ...
        for job in sorted_jobs:
            ...
            resource_name = resource_name[:30] + "..." if len(resource_name) > 30 else resource_name
            ...
            pdf.cell(col_widths[4], h, creation_time.strftime("%Y-%m-%d %H:%M"), border=1, align="C")
        ...
        for vault_name, jobs in backup_data.items():
            ...
            for job in failed:
                ...
                if job.get("StatusMessage"):
                    pdf.multi_cell(0, 5, f"Error: {job.get('StatusMessage', 'No error message')}")
                pdf.ln(4)
...
def lambda_handler(event, context):
    ...
    safe_customer = (
        ...
        .replace("/", "")
        .replace("\\", "")
    ) if CUSTOMER_IDENTIFIER else "backup"
    
    key = f"{year}/{month}/{safe_customer}-backup-status-report-{year}-{month}-{day}.pdf"
